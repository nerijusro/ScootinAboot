...
func TestScooterHandler(t *testing.T) {
	scootersRepository := &mockScooterRepository{}
	validator := &mockScooterRequestValidator{}
	scootersHandler := NewScooterHandler(scootersRepository, validator)
	...
	t.Run("When getting scooters by area while everything is valid returns ok", func(t *testing.T) {
		request, err := http.NewRequest(http.MethodGet, "/client/scooters?availability=available&x1=25.0&x2=26.0&y1=54.0&y2=55.0", nil)
		if err != nil {
			t.Fatal(err)
		}

		router := gin.Default()
		router.GET("/client/scooters", scootersHandler.getScootersByArea)
		...
		var response types.GetScootersResponse
		if err := json.NewDecoder(responseRecoreder.Body).Decode(&response); err != nil {
			t.Fatal(err)
		}

		scooter := response.Scooters[0]
		if scooter.Location.Latitude != 12.12 {
			t.Errorf("expected latitude to be: 12.12, got %f", scooter.Location.Latitude)
		}

		if scooter.Location.Longitude != 44.34 {
			t.Errorf("expected longitude to be 44.34, got %f", scooter.Location.Longitude)
		}

		if scooter.IsAvailable != true {
			t.Errorf("expected availability to be true, got %t", scooter.IsAvailable)
		}
	})

	t.Run("When getting scooters by area while query parameters are not valid returns bad request", func(t *testing.T) {
		request, err := http.NewRequest(http.MethodGet, "/client/scooters?invalidParameter=some-invalid-param", nil)
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("When getting scooters by area while query parameter content is not valid returns bad request", func(t *testing.T) {
		request, err := http.NewRequest(http.MethodGet, "/client/scooters?availability=noSuchThingAsAvailable&x1=25.0&x2=26.0&y1=54.0&y2=55.0", nil)
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("When getting all scooters while everything is valid returns ok", func(t *testing.T) {
		...
		firstScooter := response.Scooters[0]
		if firstScooter.Location.Latitude != 12.12 {
			t.Errorf("expected latitude to be: 12.12, got %f", firstScooter.Location.Latitude)
		}

		if firstScooter.Location.Longitude != 44.34 {
			t.Errorf("expected longitude to be 44.34, got %f", firstScooter.Location.Longitude)
		}

		if firstScooter.IsAvailable != true {
			t.Errorf("expected availability to be true, got %t", firstScooter.IsAvailable)
		}
	})

	t.Run("When getting scooter by id while everything is valid returns ok", func(t *testing.T) {
		request, err := http.NewRequest(http.MethodGet, "/client/scooters/e3344268-d649-4c19-a20c-a0c64a5a6623", nil)
		if err != nil {
			t.Fatal(err)
		}

		router := gin.Default()
		router.GET("/client/scooters/:id", scootersHandler.getScooter)
	...
	t.Run("When getting scooter by id while scooter is not existant returns not found", func(t *testing.T) {
		request, err := http.NewRequest(http.MethodGet, "/client/scooters/e3344268-1234-4c19-a20c-a0c64a5a6623", nil)
		if err != nil {
			t.Fatal(err)
		}
